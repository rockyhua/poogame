...
public:
    ...
    /**
     * @brief clone class will deep copy a single stage2 ball object
     * @return copyBall
     */
    Prototype* clone(){
        Stage2Ball* copyBall = new Stage2Ball();
        copyBall->setColour(m_colour);
        copyBall->setMass(m_mass);
        copyBall->setPosition(m_position);
        copyBall->setRadius(m_radius);
        copyBall->setVelocity(m_velocity);
        copyBall->setStrength(m_strength);
        return copyBall;
    }
    // Ball interface
public:
...
public:
    ...
    /**
     * @brief clone class will deep copy a single Composite ball object
     * @return copyBall
     */
    Prototype* clone(){
        CompositeBall* copyBall = new CompositeBall();
        copyBall->setColour(m_colour);
        copyBall->setMass(m_mass);
        copyBall->setPosition(m_position);
        copyBall->setRadius(m_radius);
        ...
        for (Ball* cball : m_containedBalls){
            Ball* nb = (Ball*)cball->clone();
            copyBall->addBall(nb);
        }
        return copyBall;
    }
