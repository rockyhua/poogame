...
void PoolGame::simulateTimeStep(float timeStep)
{
    ...
    {
        /*if one player shoot a ball into a pocket the score will increase, and that player can have a bouns shot
        *if the ball is a cueball then the score will decrease
        */
        if(player==2){
            if(e->colour()!="#ffffff"){
                player1_score++;
...
void PoolGame::setBalls(std::vector<Ball*> balls){
    m_balls = balls;
}

std::vector<Ball*> PoolGame::getBalls(){
    return m_balls;
}

void PoolGame::setTable(Table* table){
    m_table = table;
}

Table* PoolGame::getTable(){
    return m_table;
}
